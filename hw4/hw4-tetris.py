...
pieceSequence = np.tile(np.array([1, 2, 0]), 100)
contPlan  = np.zeros((3,numRounds, 512), dtype=int)
costToGo = np.zeros((numRounds, 512), dtype=np.float)

validStatesList = validStates

Pb = np.array([[0,1,0],[0,0,1],[1,0,0]])
Pc = 1./3*np.ones((3,3), dtype=np.float)

for rnd in range(numRounds-2, -1, -1):
    if rnd == 0:
        validStatesList = [0]
    for state in range(0, 512):
        if not state in validStatesList:
            costToGo[rnd,state] = 100
            continue
        minVal = np.zeros((3,), dtype=int)
        for piece in pieces:
            pieceWasUpdated = False
            for nextState in possibleNextStates(state, piece):
                ...
                if (not pieceWasUpdated) or \
                        costOfNext < minVal[piece]:
                    pieceWasUpdated = True
                    contPlan[piece, rnd, state] = cleanNext
                    minVal[piece] = costOfNext
        # costToGo[rnd, state] = 1./3*np.sum(np.dot(Pb, minVal))
        costToGo[rnd, state] = 1./3*np.sum(np.dot(Pc, minVal))

for rnd in range(0, numRounds):
    state0 = contPlan[0, 0, 0]
    state1 = contPlan[1, 0, 0]
    state2 = contPlan[2, 0, 0]

    piece = pieceSequence[rnd]
    print(format(state0, '09b'), '\tPiece: ',
            1, '\tCost: ', costToGo[rnd, state0])

    print(format(state1, '09b'), '\tPiece: ',
            2, '\tCost: ', costToGo[rnd, state1])

    print(format(state2, '09b'), '\tPiece: ',
            3, '\tCost: ', costToGo[rnd, state2])
